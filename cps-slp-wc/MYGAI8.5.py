...
class cpstopoFakeScheduling:
    ...
    def calculateFakeSource(self, node, Ti):
        ...
        sifenmu = self.w_2 * np.exp(numI + ave_dist) + (1 - self.w_2) * np.exp(CP - numC * 1. / numI)
...
if __name__ == '__main__':
    ...
    for i in range(50):
        ...
        fs = cpstopoFakeScheduling(G=network,
                                   Tmax=5000, c_capture=1e-40, w_1=0.01, w_2=0.6,
    ...
    plt.legend()
    plt.savefig(r'D:\project\cps-slp-wc\graph\three\w_1\8.5\w_1=0.01-w_2=0.06-2.png')
    # plt.show()
