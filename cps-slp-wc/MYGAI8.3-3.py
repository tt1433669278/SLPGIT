                ...
                for i in range(n):
                    for j in range(0, n - i - 1):
                        if self.G.nodeList[a[j]].energy < self.G.nodeList[a[j + 1]].energy:  # 如果前面的元素比后面的元素小，则交换它们的位置
                            self.G.nodeList[a[j]].energy, self.G.nodeList[a[j + 1]].energy = self.G.nodeList[
                                                                                                 a[j + 1]].energy, \
                                                                                             self.G.nodeList[
                                                                                                 a[j]].energy
    ...
    def sourcetohuan(self):
        ...
        for i in range(n):
            for j in range(len(self.phantom) - i - 1):
                if self.G.nodeList[j].level < self.G.nodeList[j + 1].level:
                    self.G.nodeList[j].level, self.G.nodeList[j + 1].level = self.G.nodeList[j + 1].level, \
                                                                             self.G.nodeList[j].level
        for v in range(n):
...
if __name__ == '__main__':
    ...
    for i in range(10):
        ...
        fs = cpstopoFakeScheduling(G=network,
                                   Tmax=4000, c_capture=1e-40, w_1=0.01, w_2=0.01,
    ...
    plt.legend()
    plt.savefig(r'D:\project\cps-slp-wc\graph\three\w_1\8.1\w_1=0.01-w_2=0.01-2.png')
    # plt.show()
