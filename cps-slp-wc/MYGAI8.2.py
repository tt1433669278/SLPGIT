...
class cpstopoFakeScheduling:
                ...
                for i in range(n):
                    for j in range(0, n - i - 1):
                        if self.G.nodeList[a[j]].energy < self.G.nodeList[a[j + 1]].energy:  # 如果前面的元素比后面的元素小，则交换它们的位置
                            self.G.nodeList[a[j]].energy, self.G.nodeList[a[j + 1]].energy = self.G.nodeList[a[j + 1]].energy, self.G.nodeList[a[j]].energy

                # print a
                top_10_elements = self.open_set
    ...
    def phantonm_node(self):
        ...
        for i in phantom_set:
            ...
            if s_alpha > abs(sata) and s_beta > abs(sata):
    ...
    def sourcetohuan(self):
        ...
        if len(self.phantom) == 0:
            # print "self.phantom == 0:"
            self.phantom = list(self.phantom_sum)
        phantom_node = random.choice(self.phantom)
        ...
        self.sum_path = self.phantomtosink_path
    ...
    def calculateFakeSource(self, node, Ti):
        ...
        fenmu = self.w_2 * E + (1 - self.w_2)*(math.log(ave_dist, 2.7) + np.exp(CP - numC * 1. / numI))
        p_i = fenzi/fenmu
...
if __name__ == '__main__':
    ...
    for i in range(100):
        ...
        for v in range(10):
            ...
            print "##################the", v, "次", network.nodeNumber, network.areaLength, "w_1=0.01", "w_2=", w_2 # ...................
            fs = cpstopoFakeScheduling(G=network,
                                       Tmax=4000, c_capture=1e-40, w_1=0.01, w_2=w_2,  # ....................
            ...
            restEnergy = [fs.G.initEnergy - node.energy for node in fs.G.nodeList if
                          node.identity != fs.source and node.identity != fs.sink and node.identity != 2]
        ...
        hunsafe_plt.append(np.mean(ave_safe_set))
        hundelay_plt.append(np.mean(ave_delay_set))
        hunenergy_plt.append(np.mean(ave_energy_set))
        print "The safety is:", np.mean(ave_safe_set), "The meanrestEnergy is:", np.mean(ave_energy_set), "The MeanDelay is:", np.mean(ave_delay_set)
    print "\nThe ave_safety is:", np.mean(hunsafe_plt), "The meanrestEnergy is:", np.mean(hunenergy_plt), "The MeanDelay is:", np.mean(hundelay_plt)
    ...
    plt.legend()
    plt.savefig(r'D:\project\cps-slp-wc\graph\three\w_1\8.1\w_1=0.01.png')
    plt.show()
