...
if __name__ == '__main__':
    ...
    for i in range(5):
        ...
        print "第", i, "轮.................网络规模：", network.nodeNumber, network.areaLength, "w_1=0.01", "w_2=", w_2
        ...
        restEnergy = [fs.G.initEnergy - node.energy for node in fs.G.nodeList if
                      node.identity != fs.source and node.identity != fs.sink and node.energy > 0]
        # for node in fs.G.nodeList:
        #     if node.identity != fs.source and node.identity != fs.sink and node.energy > 0:


        hunsafe_plt.append(fs.safety)
        hundelay_plt.append(np.mean(fs.listDelay))
        hunenergy_plt.append(np.mean(restEnergy))
        print "The safety is:", fs.safety, "The meanrestEnergy is:", np.mean(restEnergy), "The MeanDelay is:", np.mean(fs.listDelay)
    print "\nThe ave_safety is:", np.mean(hunsafe_plt), "The neanrestEnergy is:", np.mean(hunenergy_plt), "The MeanDelay is:", np.mean(hundelay_plt)
