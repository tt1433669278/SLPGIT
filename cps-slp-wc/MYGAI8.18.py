...
class cpstopoFakeScheduling:
    ...
    def calculateFakeSource(self, node, Ti):
        ...
        # 第四版
        sifenzi = self.w_1 * np.exp(numB * 1. / numI)*e*0.1
        sifenmu = self.w_2 * math.log((ave_dist + numI), 2.7) + (1 - self.w_2) * math.log(node_dise, 2.7)
        sifenmu = self.w_2 * (ave_dist + numI) + (1 - self.w_2) * node_dise
...
if __name__ == '__main__':
    ...
    for i in range(9):
        ...
        plt.savefig(r'D:\project\cps-slp-wc\graph\three\w_1\8.6\8.10-3400-w_2-{}.png' .format(w_2))
        plt.clf()
        # fs.attacker.display()
