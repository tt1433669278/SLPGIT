...
class cpstopoFakeScheduling:
    ...
    def phantonm_node(self):
        ...
        for v in self.G.nodeList:
            if (self.G.nodeList[self.sink].level + 11) < v.level < (self.G.nodeList[self.source].level):
        ...
        for i in phantom_set:
            ...
            s_alpha = math.degrees(math.acos((H * H + Y * Y - L * L) / (2 * H * Y)))
            s_beta = math.degrees(math.acos((H * H + L * L - Y * Y) / (2 * H * L)))
            beta_set.append(s_beta)
            if s_alpha > sata and s_beta > sata:
                if self.G.nodeList[i].position[1] - self.G.nodeList[self.source].position[1] > 0 and self.G.nodeList[i].position[0] - self.G.nodeList[self.source].position[0] < 0:
                    set_1.append(i)
                if self.G.nodeList[i].position[1] - self.G.nodeList[self.source].position[1] < 0 and self.G.nodeList[i].position[0] - self.G.nodeList[self.source].position[0] < 0:
                    if abs(self.G.nodeList[i].position[1] - self.G.nodeList[self.source].position[1]) - abs(self.G.nodeList[i].position[0] - self.G.nodeList[self.source].position[0]) < 0:
                        set_2.append(i)
                    else:
                        set_3.append(i)
                if self.G.nodeList[i].position[1] - self.G.nodeList[self.source].position[1] < 0 and self.G.nodeList[i].position[0] - self.G.nodeList[self.source].position[0] > 0:
        ...
        for m in range(len(set_1)):
            for n in range(len(set_1) - m - 1):
                if self.G.nodeList[set_1[n]].level > self.G.nodeList[set_1[n + 1]].level:
                    set_1[n], set_1[n + 1] = set_1[n + 1], set_1[n]
        for m in range(len(set_2)):
            for n in range(len(set_2) - m - 1):
                if self.G.nodeList[set_2[n]].level > self.G.nodeList[set_2[n + 1]].level:
                    set_2[n], set_2[n + 1] = set_2[n + 1], set_2[n]
        for m in range(len(set_3)):
            for n in range(len(set_3) - m - 1):
                if self.G.nodeList[set_3[n]].level > self.G.nodeList[set_3[n + 1]].level:
                    set_3[n], set_3[n + 1] = set_3[n + 1], set_3[n]
        for m in range(len(set_3)):
            for n in range(len(set_3) - m - 1):
                if self.G.nodeList[set_3[n]].level > self.G.nodeList[set_3[n + 1]].level:
                    set_3[n], set_3[n + 1] = set_3[n + 1], set_3[n]
        set_sum = [len(set_1), len(set_2), len(set_3), len(set_4)]
        max_set = max(set_sum)
        b_1, b_2, b_3, b_4 = 0, 0, 0, 0
        for b in range(max_set):
            if len(set_1) > 0:
                if b_1 < len(set_1):
                    zong.append(set_1[b_1])
                    b_1 += 1
                else:
                    b_1 = 0
                    zong.append(set_1[b_1])
            if b_2 < len(set_2):
                zong.append(set_2[b_2])
                b_2 += 1
            else:
                b_2 = 0
                zong.append(set_2[b_2])
            if b_3 < len(set_3):
                zong.append(set_3[b_3])
                b_3 += 1
            else:
                b_3 = 0
                zong.append(set_3[b_3])
            if len(set_4) > 0:
                if b_4 < len(set_4):
                    zong.append(set_4[b_4])
                    b_4 += 1
                else:
                    b_4 = 0
                    zong.append(set_4[b_4])
        self.phantom_sum = list(zong)
    ...
    def calculateFakeSource(self, node, Ti):
        ...
        e = round(node.energy / self.G.initEnergy, 4)
        ...
        # 第四版
        sifenzi = self.w_1 * np.exp(numB * 1. / numI)*e
        sifenmu = self.w_2 * math.log((ave_dist + numI), 2.7) + (1 - self.w_2) * math.log(node_dise, 2.7)
...
if __name__ == '__main__':
    ...
    for i in range(9):
        ...
        for j in range(10):
            ...
            print "THE   w_1 is", w_1, "w_2 is", w_2
            ave_safe_set = []
            ave_energy_set = []
            ave_delay_set = []
            ave_Consump_set = []
            for v in range(20):
                ...
                fs = cpstopoFakeScheduling(G=network,
                                           Tmax=5000, c_capture=1e-40, w_1=w_1, w_2=w_2,
                ...
                ave_Consump_set.append(np.mean(fs.listEnergyConsumption))

            print "\nThe safety is", ave_safe_set
            print "\nThe meansafety is", np.mean(ave_safe_set)
            # print "\nThe Delay is", ave_delay_set, "\nThe restEnergy is", ave_energy_set
            # print "\nThe meanrestEnergy is", np.mean(ave_energy_set), "\nThe meanConsump is", np.mean(ave_Consump_set), "\nThe meandelay is", np.mean(ave_delay_set)
            safe_set.append(np.mean(ave_safe_set))
            energy_set.append(np.mean(ave_energy_set))
            delay_set.append(np.mean(ave_delay_set))
            Consump_set.append(np.mean(ave_Consump_set))
        print "\nThe everysafety is", safe_set
        # print "\nThe everyDelay is", delay_set, "\nThe everyrestEnergy is", energy_set, "\nThe everycosum is", Consump_set
        plt.figure(figsize=(20, 20))
        plt.subplot(221)
        plt.plot(w_1_values, safe_set, 'g--o', label='hunsafe_plt')
        plt.xlabel('w_1')
        plt.ylabel('Values')
        plt.legend()
        plt.title('w_2: %.2f Variation of safe with _1' % w_2)  # ............................
        plt.subplot(222)
        plt.plot(w_1_values, energy_set, 'b-', label='hunenergy_plt')
        plt.xlabel('w_1')
        plt.ylabel('Values')
        plt.title('w_2:%.2f Variation of energy with w_2' % w_2)  # ............................
        plt.legend()
        plt.subplot(223)
        plt.plot(w_1_values, delay_set, 'r:.', label='hundelay_plt')
        plt.xlabel('w_1')
        plt.ylabel('Values')
        plt.title('w_2:%.2f Variation of delay with w_2' % w_2)  # ............................
        plt.legend()
        plt.subplot(224)
        plt.plot(w_1_values, Consump_set, 'r:.', label='hunCnsum_plt')
        plt.xlabel('w_1')
        plt.ylabel('Values')
        plt.title('w_2:%.2f Variation of cosum with w_2' % w_2)  # ............................
        plt.legend()
        plt.suptitle("w_2:%.2f" % w_2)
        plt.savefig(r'D:\project\cps-slp-wc\graph\three\w_1\8.6\8.10w_2-{}.png' .format(w_2))
        plt.clf()
        # fs.attacker.display()
    ...
    # plt.legend()
    # plt.savefig(r'D:\project\cps-slp-wc\graph\three\w_1\8.6\w_1=0.08-w_2=0.9.png')
    # # plt.show()
